...
module.exports = function (stream) {
	return new Promise(function(resolve, reject) {
		parser(stream).then(function(results) {
			...
			// Opentype parser seems to get oblique wrong for everything:
			// We'll just make all 'oblique' detected fonts normal, and 
			// not properly handle true oblique fonts.
			var style = fontStyle(results['OS/2'].selection);
			if(style === 'oblique') style = 'normal';
